...
注意截图里的细节:机票和酒店价格都带「浏览器实查 · 2026-06-12」时间戳,同一班机给出携程/去哪儿两个台面价;读不到的数字(华住会 App 会员价、需登录的平台)如实标「未核实」——**没核实的数字长得就和核实过的不一样**。实查还当场推翻了两个想当然:这趟日期在售的直飞全落天府机场(初稿想当然推荐双流),初稿里推荐的一家如家分店在携程**查无此店**,行程和住宿卡都被改掉了。这就是"查证型行程页"和"写出来的攻略"的区别。
